...
from OpusV.utils.admin_filters import dev_filter, admin_filter, sudo_filter
from OpusV.utils.database import get_assistant
...
async def join_group(app, message):
    ...
    try:
        ...
        if chat_member.status != ChatMemberStatus.ADMINISTRATOR:
            return await status_message.edit("❌ I need to be an admin to invite the assistant.")
...
async def leave_all(app, message):
    left = 0
    failed = 0
    status_message = await message.reply("🔄 Assistants are  leaving all chats...")

    try:
        userbot = await get_assistant(message.chat.id)
        async for dialog in userbot.get_dialogs():
            if dialog.chat.id == -1002255890225:
                continue
            try:
