import asyncio

from pyrogram.enums import ChatMemberStatus
from pyrogram.errors import (
    ChatAdminRequired,
    ChatWriteForbidden,
    InviteHashExpired,
    InviteRequestSent,
    UserAlreadyParticipant,
    UserNotParticipant,
)
from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup

from config import PLAYLIST_IMG_URL, SUPPORT_CHAT, adminlist
from strings import get_string
from OpusV import YouTube, app
from OpusV.misc import SUDOERS
from OpusV.utils.database import (
    get_assistant,
    get_cmode,
    get_lang,
    get_playmode,
    get_playtype,
    is_active_chat,
    is_maintenance,
)
from OpusV.utils.inline import botplaylist_markup

links = {}


async def safe_reply(message, text, reply_markup=None, **kwargs):
    """Safely reply to message with error handling"""
    try:
        return await message.reply_text(text, reply_markup=reply_markup, **kwargs)
    except ChatWriteForbidden:
        print(f"Cannot send message in chat {message.chat.id}")
        return None
    except Exception as e:
        print(f"Error sending message: {e}")
        return None


async def safe_reply_photo(message, photo, caption, reply_markup=None, **kwargs):
    """Safely reply with photo"""
    try:
        return await message.reply_photo(photo=photo, caption=caption, reply_markup=reply_markup, **kwargs)
    except ChatWriteForbidden:
        print(f"Cannot send photo in chat {message.chat.id}")
        return None
    except Exception as e:
        print(f"Error sending photo: {e}")
        return None


async def check_bot_permissions(chat_id):
    """Check if bot has necessary permissions"""
    try:
        bot_member = await app.get_chat_member(chat_id, app.me.id)
        if bot_member.status in (ChatMemberStatus.BANNED, ChatMemberStatus.RESTRICTED):
            return False
        # Check if bot has admin privileges or at least can invite users
        can_manage = bot_member.privileges and (
            bot_member.privileges.can_manage_chat or 
            bot_member.privileges.can_invite_users
        )
        return can_manage
    except ChatAdminRequired:
        return False
    except Exception:
        return False


def PlayWrapper(command):
    async def wrapper(client, message):
        language = await get_lang(message.chat.id)
        _ = get_string(language)

        # Early bot status check
        try:
            bot_member = await app.get_chat_member(message.chat.id, app.me.id)
            if bot_member.status in (ChatMemberStatus.BANNED, ChatMemberStatus.RESTRICTED):
                return  # Bot is banned/restricted, exit silently
        except Exception:
            pass  # Continue if we can't check status

        if message.sender_chat:
            upl = InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="ʜᴏᴡ ᴛᴏ ғɪx ?",
                            callback_data="AnonymousAdmin",
                        ),
                    ]
                ]
            )
            return await safe_reply(message, _["general_3"], reply_markup=upl)

        if await is_maintenance() is False:
            if message.from_user.id not in SUDOERS:
                return await safe_reply(
                    message,
                    text=f"{app.mention} ɪs ᴜɴᴅᴇʀ ᴍᴀɪɴᴛᴇɴᴀɴᴄᴇ, ᴠɪsɪᴛ <a href={SUPPORT_CHAT}>sᴜᴘᴘᴏʀᴛ ᴄʜᴀᴛ</a> ғᴏʀ ᴋɴᴏᴡɪɴɢ ᴛʜᴇ ʀᴇᴀsᴏɴ.",
                    disable_web_page_preview=True,
                )

        try:
            await message.delete()
        except Exception:
            pass

        audio_telegram = (
            (message.reply_to_message.audio or message.reply_to_message.voice)
            if message.reply_to_message
            else None
        )
        video_telegram = (
            (message.reply_to_message.video or message.reply_to_message.document)
            if message.reply_to_message
            else None
        )
        url = await YouTube.url(message)

        if audio_telegram is None and video_telegram is None and url is None:
            if len(message.command) < 2:
                if "stream" in message.command:
                    return await safe_reply(message, _["str_1"])
                buttons = botplaylist_markup(_)
                return await safe_reply_photo(
                    message,
                    photo=PLAYLIST_IMG_URL,
                    caption=_["play_18"],
                    reply_markup=InlineKeyboardMarkup(buttons),
                )

        if message.command[0][0] == "c":
            chat_id = await get_cmode(message.chat.id)
            if chat_id is None:
                return await safe_reply(message, _["setting_7"])
            try:
                chat = await app.get_chat(chat_id)
            except Exception:
                return await safe_reply(message, _["cplay_4"])
            channel = chat.title
        else:
            chat_id = message.chat.id
            channel = None

        playmode = await get_playmode(message.chat.id)
        playty = await get_playtype(message.chat.id)
        if playty != "Everyone":
            if message.from_user.id not in SUDOERS:
                admins = adminlist.get(message.chat.id)
                if not admins:
                    return await safe_reply(message, _["admin_13"])
                elif message.from_user.id not in admins:
                    return await safe_reply(message, _["play_4"])

        if message.command[0][0] == "v":
            video = True
        else:
            if "-v" in message.text:
                video = True
            else:
                video = True if message.command[0][1] == "v" else None

        if message.command[-1] == "e":
            if not await is_active_chat(chat_id):
                return await safe_reply(message, _["play_16"])
            fplay = True
        else:
            fplay = None

        if not await is_active_chat(chat_id):
            userbot = await get_assistant(chat_id)
            
            # Check bot permissions first
            has_permissions = await check_bot_permissions(chat_id)
            
            try:
                try:
                    member = await app.get_chat_member(chat_id, userbot.id)
                except ChatAdminRequired:
                    if has_permissions:
                        return await safe_reply(message, _["call_1"])
                    else:
                        # Bot doesn't have permissions to check or manage
                        print(f"Bot lacks permissions in chat {chat_id}")
                        return await safe_reply(message, "Bot needs admin permissions to manage video/voice chats.")

                if member.status in (
                    ChatMemberStatus.BANNED,
                    ChatMemberStatus.RESTRICTED,
                ):
                    return await safe_reply(
                        message,
                        _["call_2"].format(
                            app.mention, userbot.id, userbot.name, userbot.username
                        ),
                        reply_markup=InlineKeyboardMarkup(
                            [
                                [
                                    InlineKeyboardButton(
                                        text="𝗨ɴʙᴀɴ 𝗔ssɪsᴛᴀɴᴛ 🔈",
                                        callback_data="unban_assistant",
                                    )
                                ]
                            ]
                        ),
                    )
            except UserNotParticipant:
                if chat_id in links:
                    invitelink = links[chat_id]
                else:
                    if message.chat.username:
                        invitelink = message.chat.username
                        try:
                            await userbot.resolve_peer(invitelink)
                        except Exception:
                            pass
                    else:
                        try:
                            invitelink = await app.export_chat_invite_link(chat_id)
                        except ChatAdminRequired:
                            return await safe_reply(message, _["call_1"])
                        except Exception as e:
                            return await safe_reply(
                                message,
                                _["call_3"].format(app.mention, type(e).__name__)
                            )

                if invitelink.startswith("https://t.me/+"):
                    invitelink = invitelink.replace(
                        "https://t.me/+", "https://t.me/joinchat/"
                    )

                myu = await safe_reply(message, _["call_4"].format(app.mention))
                if myu is None:  # Couldn't send message
                    return
                
                try:
                    await asyncio.sleep(1)
                    await userbot.join_chat(invitelink)
                except InviteHashExpired:
                    if chat_id in links:
                        del links[chat_id]
                    try:
                        invitelink = await app.export_chat_invite_link(chat_id)
                    except ChatAdminRequired:
                        return await safe_reply(message, _["call_1"])
                    except Exception as e:
                        return await safe_reply(
                            message,
                            _["call_3"].format(app.mention, type(e).__name__)
                        )
                    if invitelink.startswith("https://t.me/+"):
                        invitelink = invitelink.replace(
                            "https://t.me/+", "https://t.me/joinchat/"
                        )
                    links[chat_id] = invitelink
                    await userbot.join_chat(invitelink)
                except InviteRequestSent:
                    try:
                        await app.approve_chat_join_request(chat_id, userbot.id)
                    except Exception as e:
                        return await safe_reply(
                            message,
                            _["call_3"].format(app.mention, type(e).__name__)
                        )
                    await asyncio.sleep(2)
                    try:
                        await myu.edit(_["call_5"].format(app.mention))
                    except Exception:
                        await safe_reply(message, _["call_5"].format(app.mention))
                except UserAlreadyParticipant:
                    pass
                except Exception as e:
                    return await safe_reply(
                        message,
                        _["call_3"].format(app.mention, type(e).__name__)
                    )

                links[chat_id] = invitelink

                try:
                    await userbot.resolve_peer(chat_id)
                except Exception:
                    pass

        return await command(
            client, message, _, chat_id, video, channel, playmode, url, fplay
        )

    return wrapper
